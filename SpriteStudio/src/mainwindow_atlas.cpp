...
void MainWindow::removeAtlasBackgroundAndRefresh() {
    ...
    ui->backgroundRemoval->setChecked(true);
    removeAtlasBackground();

    QImage image = extractor->m_atlas;
    ...
    if (spriteExt) {
        ...
        QGraphicsPixmapItem *item = sceneLayers->addPixmap(QPixmap::fromImage(extractor->m_atlas));
...
void MainWindow::removeAtlasBackground()
{
    ...
    statusLabel->setText(tr("_delete_background") + " ...");;
    QImage image = extractor->m_atlas;
    image = image.convertToFormat(QImage::Format_ARGB32);
    ...
    for (int y = 0; y < image.height(); y += 2) {
        ...
        progressBar->setValue(100*y/image.height());
    }
...
void MainWindow::setupGraphicsView(const QImage &pixmap)
{
    ...
    QGraphicsPixmapItem *atlasItem = new QGraphicsPixmapItem(QPixmap::fromImage(pixmap));
